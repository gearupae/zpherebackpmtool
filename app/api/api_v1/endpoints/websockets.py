...
class ConnectionManager:
    def __init__(self):
        ...
        # Store chat room connections by room_id
    ...
    def disconnect(self, user_id: str, organization_id: str = None, project_id: str = None, task_id: str = None, room_id: str = None):
        ...
        if room_id and room_id in self.chat_connections:
            self.chat_connections[room_id].pop(user_id, None)
            if not self.chat_connections[room_id]:
                del self.chat_connections[room_id]
    ...
    async def connect_to_chat_room(self, websocket: WebSocket, user_id: str, room_id: str):
        """Connect a user to a chat room"""
        await websocket.accept()
        if room_id not in self.chat_connections:
            self.chat_connections[room_id] = {}
        self.chat_connections[room_id][user_id] = websocket

    async def send_to_chat_room(self, message: dict, room_id: str, exclude_user: str = None):
        """Send a message to all users connected to a chat room"""
        if room_id in self.chat_connections:
            for user_id, connection in list(self.chat_connections[room_id].items()):
                if exclude_user and user_id == exclude_user:
                    continue
                try:
                    await connection.send_text(json.dumps(message))
...
async def websocket_chat_room(websocket: WebSocket, room_id: str, token: str, db: AsyncSession = Depends(get_db)):
    """WebSocket endpoint for org chat rooms"""
    try:
        user = await get_current_user_ws(token, db)
        ...
        result = await db.execute(
            select(ChatRoom).where(
                ChatRoom.id == room_id,
                ChatRoom.organization_id == user.organization_id
            )
        )
        ...
        # Connect to room
        await manager.connect_to_chat_room(websocket, user.id, room_id)

        # Acknowledge
        await websocket.send_text(json.dumps({
            "type": "connected",
            "message": f"Connected to room: {room.name}",
            "room_id": room_id,
            "user_id": user.id,
            "timestamp": datetime.utcnow().isoformat()
        }))
        ...
        while True:
            ...
            if message_data.get("type") == "ping":
                ...
                await manager.send_to_chat_room(typing_msg, room_id, exclude_user=user.id)

    except WebSocketDisconnect:
        manager.disconnect(user.id, room_id=room_id)
    except Exception:
        await websocket.close(code=1000)
...
async def send_chat_message(room_id: str, message: dict):
    """Broadcast a chat message to a room (used by HTTP endpoints)."""
    await manager.send_to_chat_room(message, room_id)
